...
// ─── GC9A01A init sequence (Waveshare ESP32-S3-LCD-1.28) ─────────────────────
// Source: Waveshare demo code + GC9A01A datasheet
...
// ─── Framebuffer — 240×240 × 2 bytes = 115.2 KB  (fits even in internal RAM) ─
#define FB_W  240
#define FB_H  320
...
// Big-endian swap needed before sending to GC9A01A
...
esp_err_t display_init(void)
{
    ...
    // Framebuffer — try PSRAM first, fall back to internal DRAM
...
{
    const int STRIP = 40;
    static uint16_t strip_buf[FB_W * 40];   // static = stack-safe
    ...
    for (int y = 0; y < FB_H; y += STRIP) {
        ...
        // Byte-swap to big-endian for GC9A01A
...
// Arc: angle 0° = right, positive = counter-clockwise
...
// ─── 5×7 pixel font — digits 0-9, decimal point, 'W' ────────────────────────
static const uint8_t FONT5X7[][5] = {
    ...
    {0x00,0x41,0x7F,0x41,0x00}, // 'I' index 12  (used for "BLE" indicator)
...
// ─── Screen state ─────────────────────────────────────────────────────────────
typedef enum { SCR_CDA=0, SCR_POWER, SCR_STATUS, SCR_COUNT } screen_t;
static screen_t g_screen = SCR_CDA;

// ─── Render ───────────────────────────────────────────────────────────────────
void display_render(const aerodrag_sensors_t *s, const aerodrag_physics_t *p)
{
    if (!g_fb) return;
    const int CX = FB_W/2, CY = FB_H/2;   // 120, 120

    fb_fill(COL_BG);

    // Outer bezel ring (full circle minus small gap at bottom)
...
{
    g_screen = (screen_t)((g_screen + 1) % SCR_COUNT);
}
