...
Componente gestito aggiuntivo: **sensore ruota Crr** (firmware nel repo dedicato
**`aerodrag-wheel`**, nRF52840) — repo figlia gestita dal contratto, vedi confine G.
*(v0.3.1: spostato da `wheel-fw/` nella repo madre → repo `aerodrag-wheel`.)*
...
- **(v0.3.1, audit) Obbligo lato app**: l'app **non deve tentare** la WRITE se il proprio
  clock è `< 2020` (`epochMs < 1577836800000`) — eviterebbe l'errore ATT `VALUE_NOT_ALLOWED`
  e in quel caso lascia `tUtc=0` (fallback `serverTs`); quando il clock è valido la WRITE è
  **obbligatoria on-connect**.
...
gestito** dal contratto: vive nel repo dedicato **`aerodrag-wheel`** (firmware
separato, **non** parte del build ESP-IDF; v0.3.1: era `wheel-fw/` nella madre).
Espone il servizio `0xBB00` che
...
**`pc-receiver` — parità Pi↔coach (v0.3.1, audit).** Il `/receive` del **Pi**
(`aerodrag-pi/pc-receiver`) DEVE comportarsi come quello del coach: (a) **validare lo
schema** del body **prima** di scrivere su disco (oggetto con `ts:number`, `deviceId`
MAC valido, `laps:array`) → `400` e nessun file se invalido; (b) **bindare solo
all'interfaccia USB** (es. `192.168.7.2`, override via env), **non** `0.0.0.0` (riduce
la superficie d'attacco sulla LAN). Allinearsi all'implementazione coach.
...
| sensore ruota Crr (repo `aerodrag-wheel`) | 0.3.0 — XIAO BLE Sense + LSM6DS3TR-C; bring-up funzionante, da compilare/flashare in locale |
...
nessuna modifica al wire). Da implementare nelle **chat figlie**.
- **Firmware sensore ruota → repo dedicato `aerodrag-wheel`** (era `wheel-fw/` nella
  repo madre). L'implementazione funzionante (servizio `0xBB00`, IMU LSM6DS3TR-C,
  **fondo-scala gyro ±2000 dps** — col default ±245 dps saturava a ~3 km/h, auto-asse
  di spin, batteria, persistenza) vive lì; `wheel-fw/` nella madre è **superato**
  (rimozione consigliata in un commit di pulizia). Il **wire `0xBB00` (§2.G) è invariato**.
- **app — `TIME 0xaa10` (§2)**: l'app deve **guardare il clock** e **non tentare** la
  WRITE se `epochMs < 1577836800000` (eviterebbe l'errore ATT, lascia `tUtc=0` → fallback
  `serverTs`); WRITE obbligatoria on-connect quando il clock è valido.
- **app — `tUtc` nel fallback `/coach` (§3, sim)**: anche i frame del percorso legacy
